...
class IOPSpider(XMLFeedSpider, NLM):
    ...
    1. Depending on the parameters, spider starts scraping from a local file
       or tries to connect to STACKS to get metadata for missing issues.
       You can specify a journal (by ISSN) and issue.
    ...
    .. code-block:: console
        ...
        scrapy crawl IOP -a http_netrc="tmp/stacks_netrc" -a journal_issn="1538-3881" -a issue="152/1"
    ...
    iterator = 'xml'
    itertag = 'Article'
    http_user = ''  # to enable HTTP basic authentication in Scrapy
    http_pass = ''
    ...
    def __init__(self, zip_file=None, xml_file=None, pdf_files=None,
        ...
        self.journal_issn = journal_issn # if you want a specific journal
        self.issue = issue  # if you want a specific issue
        #if journal_issn:
            #self.issns = [journal_issn]
    ...
    def scrape_for_available_issues(self, response):
        ...
        # Check if the desired ISSN is valid
        ...
        missing_issues = {}
        if self.journal_issn and self.issue:
            if self.journal_issn not in available_issues:
                raise KeyError("Journal " + self.journal_issn + " not available.")
            ...
            for journal in available_issues:
                if journal in existing_issues: # FIXME: test
                    difference = list(
        ...
        for journal_issn in missing_issues:
            for issue in missing_issues[journal_issn]:
                ...
                metadata_directory = "{}/{}/{}".format(self.local_store, journal_issn, issue)
                if not os.path.exists(metadata_directory):
                    os.makedirs(metadata_directory)
                path_to_new_file = "{}/extract.pubmed".format(metadata_directory)
                ...
                request = Request(abs_path, callback=self.parse_node)
    ...
    def handle_pdf_package(self, zip_file):
        ...
        # Preserve the dir structure inside the package
    ...
    def parse_node(self, response, node):
        ...
        pdf_files = response.meta.get("pdf_files")  # This is just the root IOP dir
        if not pdf_files and self.pdf_files:
            pdf_files = self.pdf_files
        if pdf_files:
            pdf_file_path = self.get_pdf_path(pdf_files, issn, volume, issue, fpage)
            if pdf_file_path:
